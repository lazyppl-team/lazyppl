{-# language ExtendedDefaultRules, ScopedTypeVariables, QuasiQuotes, ParallelListComp, FlexibleContexts #-}
module MhTest where
...
nnormal n mu sigma =
        do xs <- mapM (\_ -> normal (mu / n) $ sqrt (sigma ^ 2 / n)) [1 .. n]
           return $ sum xs
           
-- a simple model that has a nasty bimodal posterior
bimodal prior = do x <- sample prior
...
test model n     = do let ys = map fromIntegral $ [1..n]
                      let ideas = [(1,0,"mh 1","tab:blue"),
                      ...
                      onscreen $ histscatters xs ys (map (\(_,_,s,_)->s) ideas)
...
histscatters xs y titles xtitles colors =
  ...
  where histscatter' mpl i =
        ...
        n = length xs
        histscatter mpl i =
          if i == 0 then histscatter' mpl i % ylabel "time" else (histscatter' mpl i % mp # "ax.yaxis.set_major_formatter(mticker.NullFormatter())")
