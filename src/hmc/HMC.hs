...
simpleLeapfrog :: (Floating a, Ord a, Show a) => (a -> a) -> (LFConfig a) -> (a, a, [a], [a]) -> (a, a, [a], [a])
simpleLeapfrog dV_dq (LFConfig eps steps selected) (q, p, _, _) = let
        qps = take steps $ iterate (updateQP dV_dq eps) (q, p)
        qs = map fst qps
        ps = map snd qps
    in (last qs, last ps, qs, ps)

data HMCConfig a = HMCConfig {seed :: Int, lfc :: LFConfig a}

hmcProposal :: (Floating a, Ord a, Show a) => (LFConfig a) -> (a, a, [a], [a]) -> (a, a, [a], [a])
hmcProposal lfc (q, p, qss, pss) = let
        (q', p', qss', pss') = simpleLeapfrog dV_dq lfc (q, p, qss, pss)
    in (q', -p', qss', pss')

hmcAcceptReject :: (RandomGen g, Floating a, Ord a, Random a, Show a) => LFConfig a -> ((a, a, [a], [a]), g) -> ((a, a, [a], [a]), g)
hmcAcceptReject lfc ((q, _, qss, pss), rng) = let
        (p, rng') = normal rng -- assume the proposal distribution for momentum is N(0, 1)
        (new_q, new_p, new_qss, new_pss) = hmcProposal lfc (q, p, qss, pss)
        (u, rng'') = randomR (0.0, 1.0) rng'
        (q', p', qss', pss') = if (log u) < (-(h new_q new_p) + (h q p)) then (new_q, new_p, new_qss, new_pss) else (q, p, qss, pss)
    in ((q', p', qss', pss'), rng'')

hmc1D :: (Floating a, Ord a, Random a, Show a) => (HMCConfig a) -> [(a, a, [a], [a])]
hmc1D (HMCConfig seed lfc) = let 
        ...
        q = 1.0
        p = 1.0
    in map fst $ iterate (hmcAcceptReject lfc) ((q, p, [q], [p]), rng)

main :: IO ()
main = print $ take 1000 $ hmc1D (HMCConfig {seed=42, lfc=(LFConfig {eps=(0.05 :: Double), leapfrogSteps=100, selected=1})})
